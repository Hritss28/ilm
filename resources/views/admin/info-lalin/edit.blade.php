...
<div class="max-w-4xl mx-auto pb-20">
    ...
    <form action="{{ route('admin.info-lalin.update', $infoLalin->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-xl shadow-gray-200/50 overflow-hidden text-left">
            <div class="p-10 space-y-10">
                ...
                <div class="space-y-5">
                    ...
                    @php $currentType = $infoLalin->category ?? 'Kemacetan'; @endphp
                    <div class="flex flex-wrap gap-3" x-data="{ selected: '{{ $currentType }}', custom: {{ !in_array($currentType, ['Kemacetan','Penutupan jalan','Rute alternatif','Kecelakaan','Uji emisi']) ? 'true' : 'false' }} }">
                        ...
                        <label class="cursor-pointer" @click="selected = 'Lainnya'; custom = true; $nextTick(() => $refs.customInput.focus())">
                            <input type="radio" name="lalin_category" value="Lainnya" class="sr-only" x-bind:checked="selected === 'Lainnya' || custom">
                            <div :class="(selected === 'Lainnya' || custom) ? 'bg-gray-800 text-white border-gray-800' : 'bg-white text-gray-500 border-gray-200 hover:border-gray-400'" class="px-6 py-2.5 rounded-full border text-xs font-bold transition-all">
                                Lainnya
                            </div>
                        </label>
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="tanggal_kejadian" required class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all" value="{{ old('tanggal_kejadian', $infoLalin->incident_date?->format('Y-m-d') ?? now()->format('Y-m-d')) }}">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-gray-700">Waktu mulai <span class="text-red-500">*</span></label>
                            <input type="time" name="waktu_kejadian" required class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all" value="{{ old('waktu_kejadian', $infoLalin->start_time?->format('H:i') ?? now()->format('H:i')) }}">
                        </div>
                        <div class="space-y-2">
                            <label class="text-xs font-bold text-gray-700">Perkiraan selesai</label>
                            <input type="time" name="lalin_estimated_end" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all" value="{{ old('lalin_estimated_end', $infoLalin->estimated_end_time?->format('H:i')) }}">
                            <p class="text-[10px] text-gray-400">Kosongkan jika belum diketahui</p>
                        </div>
                        <div class="space-y-2" x-data="{ status: '{{ $infoLalin->status ?? 'Masih aktif' }}' }">
                            ...
                            <div class="flex gap-3 h-11 items-center">
                                <label class="cursor-pointer" @click="status = 'Masih aktif'">
                                    <input type="radio" name="lalin_status" value="Masih aktif" class="sr-only" x-bind:checked="status === 'Masih aktif'">
                ...
                <div class="space-y-5">
                    ...
                    <div class="space-y-4">
                        <div class="space-y-2">
                            ...
                            <input type="text" name="title" required maxlength="60" placeholder="Cth: Penutupan jalan Bypass Mojokerto" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all" value="{{ old('title', $infoLalin->title) }}">
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="lalin_source" required maxlength="100" placeholder="Cth: Dishub Surabaya" class="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm font-medium text-gray-700 focus:outline-none focus:ring-2 focus:ring-red-100 focus:border-red-300 transition-all" value="{{ old('lalin_source', $infoLalin->source) }}">
                        </div>
                    </div>
                </div>
