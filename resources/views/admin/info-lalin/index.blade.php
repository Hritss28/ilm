        ...
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50/50 border-b border-gray-100">
                    <th class="px-8 py-5 text-[10px] font-black text-gray-400 uppercase tracking-widest">Waktu / Judul</th>
                ...
                <tr class="hover:bg-gray-50/30 transition-colors group">
                    <td class="px-8 py-6">
                        <div class="flex flex-col gap-1">
                            <span class="text-[10px] font-black text-red-600 font-mono tracking-tighter uppercase">{{ $item->published_at?->format('H:i') }} &bull; {{ $item->published_at?->format('d M Y') }}</span>
                            <span class="text-xs font-bold text-gray-800 group-hover:text-red-700 transition-colors">{{ $item->title }}</span>
                        </div>
                    </td>
                    <td class="px-8 py-6 text-center">
                        <span class="inline-block px-3 py-1.5 rounded-lg bg-gray-50 text-[10px] font-black text-gray-400 uppercase border border-gray-100">
                            {{ $item->status === 'published' ? 'Published' : 'Draft' }}
                        </span>
                    </td>
                    <td class="px-8 py-6 text-xs font-bold text-gray-500">{{ number_format($item->views) }}</td>
                    <td class="px-8 py-6">
                        <div class="flex items-center justify-end gap-2 text-[10px] font-bold">
                            ...
                            <form action="{{ route('admin.news.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Hapus info lalin ini?')">
                                @csrf
                                @method('DELETE')
                ...
                <tr>
                    <td colspan="4" class="px-8 py-20 text-center">
                        <div class="flex flex-col items-center gap-2 text-gray-300">
                            ...
                            <p class="text-[10px] font-black uppercase tracking-widest">Belum ada data info lalin</p>
                        </div>
                    </td>
                </tr>
                @endforelse
