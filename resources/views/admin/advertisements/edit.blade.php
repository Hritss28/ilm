...
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold text-gray-800 mb-6">Edit Iklan: {{ $advertisement->title }}</h2>
        ...
        <form method="POST" action="{{ route('admin.advertisements.update', $advertisement) }}" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                @error('image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Kosongkan jika tidak ingin mengganti gambar. Format: JPEG, PNG, WebP, GIF. Maksimal 5MB.</p>
            </div>

            <div class="mb-4">
                <label for="link_url" class="block text-sm font-medium text-gray-700 mb-1">URL Tujuan</label>
                <input type="url" name="link_url" id="link_url" value="{{ old('link_url', $advertisement->link_url) }}" placeholder="https://..."
                    class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm"
                    required>
                @error('link_url')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <select name="position" id="position"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm"
                        required>
                        <option value="top" {{ old('position', $advertisement->position) === 'top' ? 'selected' : '' }}>Hero Kanan (2 slot)</option>
                        <option value="sidebar" {{ old('position', $advertisement->position) === 'sidebar' ? 'selected' : '' }}>Sidebar Kanan (3 slot)</option>
                        <option value="content" {{ old('position', $advertisement->position) === 'content' ? 'selected' : '' }}>Banner Horizontal (di bawah hero)</option>
                        <option value="footer" {{ old('position', $advertisement->position) === 'footer' ? 'selected' : '' }}>Footer (di atas footer)</option>
                    </select>
                    @error('position')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="priority" id="priority" value="{{ old('priority', $advertisement->priority) }}" min="0"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm"
                        required>
                    @error('priority')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" id="starts_at"
                        value="{{ old('starts_at', $advertisement->starts_at ? $advertisement->starts_at->format('Y-m-d\TH:i') : '') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                    @error('starts_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ends_at" id="ends_at"
                        value="{{ old('ends_at', $advertisement->ends_at ? $advertisement->ends_at->format('Y-m-d\TH:i') : '') }}"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-500 focus:ring-red-500 text-sm">
                    @error('ends_at')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
